module Messages
  ...
  def create_message(sender, recipient_id, token, subject, body)
    self.class.post("#{@api_url}/messages", headers: {"authorization" => @auth_token}, body: {"sender"=> sender, "recipient_id" => recipient, "token" => token, "subject" => subject, "stripped_text" => body})
  end
end
